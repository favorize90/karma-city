...
import { missions, Mission, MissionCategory } from "@/data/mockData";
    ...
    <div className="px-4 pt-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight font-display">Missionen</h1>
            <p className="mt-0.5 text-xs text-zinc-400 font-medium">Finde deinen nächsten Einsatz</p>
          </div>
      ...
      <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="relative mt-4">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="mt-3 flex gap-1.5 overflow-x-auto no-scrollbar pb-1">
        {filters.map((f, i) => (
          <motion.button
            key={f.key}
            ...
            className={`shrink-0 rounded-lg px-3 py-1.5 text-[11px] font-semibold transition ${
              activeFilter === f.key ? "bg-zinc-900 text-white" : "bg-zinc-100 text-zinc-500 hover:bg-zinc-200"
            }`}
          >
            {f.label}
          </motion.button>
      ...
      {filtered.length === 0 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mt-12 text-center">
          ...
          <button onClick={() => { setActiveFilter("all"); setSearch(""); }} className="mt-2 text-xs font-semibold text-emerald-600">
            Filter zurücksetzen
          </button>
        </motion.div>
      )}
